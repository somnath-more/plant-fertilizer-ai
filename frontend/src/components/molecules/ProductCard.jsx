  ...
  <div className="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 border border-gray-100 transform hover:-translate-y-2">
    ...
    <div className="p-6">
      <div className="flex justify-between items-start mb-3">
        <h3 className="font-poppins font-bold text-xl text-gray-900 group-hover:text-green-600 transition-colors">{product.name}</h3>
        <Badge variant={product.stock > 0 ? 'success' : 'danger'}>
          {product.stock > 0 ? 'In Stock' : 'Out of Stock'}
        </Badge>
      </div>
      <p className="text-gray-600 text-sm mb-4 font-inter leading-relaxed line-clamp-2">{product.description}</p>
      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
        <div>
          <span className="text-3xl font-bold text-green-600 font-poppins">₹{product.price}</span>
          <span className="text-gray-500 text-sm ml-2 line-through">₹{Math.round(product.price * 1.2)}</span>
        </div>
        <Button
